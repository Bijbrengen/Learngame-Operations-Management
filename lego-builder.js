(() => {
  ...
  function targetMarkup() {
    if (state.mode !== "tutorial" || state.tutorialComplete) return "";
    const expected = GOALS.A.bricks.slice(0, TUTORIAL[state.tutorialStep].expectedCount);
    return expected
  ...
  function render() {
    ...
    const instruction = state.mode === "tutorial" ? tutorial.instruction : "Bouw de bestelling exact na en kies daarna Klaar / Leveren.";
    const image = state.mode === "tutorial" ? tutorial.image : goal.image;
    container.innerHTML = `
      <div class="lego-builder-shell">
        <header class="builder-order-card">
          <div>
            <p class="eyebrow">${state.mode === "tutorial" ? `Oefentutorial · stap ${state.tutorialStep + 1}/3` : "Klantbestelling"}</p>
        ...
        <div class="builder-catalog" aria-label="Productcatalogus">
          ${Object.values(GOALS).map(product => `
            <button type="button" data-product-id="${product.name.slice(-1)}"
                    class="${state.mode === "free" && state.productId === product.name.slice(-1) ? "is-selected" : ""}"
                    ${state.mode === "tutorial" ? "disabled" : ""}>
        ...
        <div class="builder-workspace">
          <aside class="builder-palette" aria-label="Lego-blokkenpalet">
            <h4>Blokkenpalet</h4>
            <p>Klik of sleep een blok.</p>
            <div class="builder-palette-grid">${paletteMarkup()}</div>
            <button type="button" class="secondary-button builder-rotate">Draai 90°</button>
          </aside>
          ...
          <aside class="builder-actions">
            ...
            <button type="button" class="primary-button builder-deliver">${state.mode === "tutorial" && state.tutorialComplete ? "Start vrije opdracht" : "Klaar / Leveren"}</button>
